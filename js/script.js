...
const renderTheme = themes => {
  ...
  for (let i = 0; i < themes.length; i += 1) {
    ...
    if (result) {
      ...
      p.innerHTML = `
        <span class="selection__ratio">${result}/${themes[i].list.length}</span>
...
const showResult = (result, quiz) => {
  ...
  block.innerHTML = `
    ...
    <div class="result__box">
      <p class="result__ratio result__ratio--${ratio + 1}">${result}/${quiz.list.length}</p>
  ...
  button.addEventListener('click', () => {
    hideElem(block, initQuize);
  })
};

const renderQuiz = quiz => {
  const questionBox = document.createElement('div');
  questionBox.className = 'main__box main__box--question question';
